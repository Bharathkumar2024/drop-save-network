...
const DonorLayout = ({ children }: DonorLayoutProps) => {
  ...
  const menuItems = [
    { icon: Home, label: 'Dashboard', path: '/donor/dashboard-main' },
    { icon: Droplet, label: 'Donation History', path: '/donor/dashboard' },
    { icon: Calendar, label: 'Schedule', path: '/donor/dashboard' },
    { icon: Award, label: 'Reputation', path: '/donor/reputation' },
        ...
        <div className="p-4 mx-4 my-4 bg-secondary rounded-xl">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-foreground truncate">{user?.name || 'Donor'}</p>
              <p className="text-xs text-muted-foreground">Life Saver</p>
            </div>
          </div>
        </div>
